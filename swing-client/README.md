...
(Please note that the machineId on the display is the local machineId and not the same machineId as in the source ReST service)
...
mvn clean install
cd target
java -jar parkingservice-swing-client-swing-client-0.2-SNAPSHOT.one-jar.jar

```

You can also supply a local properties file 
```
java -jar parkingservice-swing-client-swing-client-0.2-SNAPSHOT.one-jar.jar -p <path to config.properties>
```
